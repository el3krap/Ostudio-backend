...
export default function ManagerDashboard({ user, onLogout }) {
  ...
  const [activeProject, setActiveProject] = useState(() => {
    try {
      const savedProj = localStorage.getItem('ostudio_active_project');
  ...
  // 1. الاستماع اللحظي لمشاريع Firestore
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(projectsQuery, (snapshot) => {
      ...
      if (activeProject) {
        ...
        if (latest) {
          ...
          localStorage.setItem('ostudio_active_project', JSON.stringify(latest));
        }
      }
    }, (err) => {
  ...
  // 2. الاستماع اللحظي لإشعارات المدير من Firestore
  useEffect(() => {
    const notifQuery = query(
      collection(db, 'notifications'),
      where('targetRole', 'in', ['manager', 'admin']),
      orderBy('createdAt', 'desc')
    );
  ...
  const handleOpenProject = (proj) => {
    ...
    localStorage.setItem('ostudio_active_project', JSON.stringify(proj));
  };
    ...
    <div className="ostudio-wrapper" style={{ direction: 'ltr', textAlign: 'left', color: '#000', position: 'relative', background: '#f8fafc', minHeight: '100vh', paddingBottom: '50px' }}>
      ...
      <nav className="ostudio-navbar" style={{ background: '#fff', borderBottom: '1px solid #e2e8f0', padding: '15px 40px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div className="nav-left-profile">
          <span style={{ fontWeight: '600', color: '#0f172a' }}>👤 {user?.name || 'Manager'} ({user?.role || 'manager'})</span>
        </div>
      ...
      <div className="ostudio-main-content" style={{ maxWidth: '850px', margin: '0 auto', padding: '0 20px' }}>
        ...
        {/* قسم إضافة المشروع مع الكلمة المطلوبة Create A New Project */}
        ...
        {showNewModal && (
          <form onSubmit={handleCreateProject} style={{ background: '#ffffff', padding: '30px', borderRadius: '12px', marginBottom: '30px', border: '1px solid #cbd5e1', boxShadow: '0 4px 15px rgba(0,0,0,0.05)', color: '#000' }}>
            ...
            <div style={{ marginBottom: '20px' }}>
              <label style={{ display: 'block', fontSize: '13px', fontWeight: 'bold', color: '#475569', marginBottom: '6px' }}>Manager Notes (Optional)</label>
              <textarea 
                placeholder="Additional instructions for designers/coordinators..." 
                value={managerNotes} 
